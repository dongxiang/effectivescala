...
<address>Marius Eriksen, Twitter Inc.<br />marius@twitter.com (<a href="http://twitter.com/marius">@marius</a>)</address><br /><br />[翻译： 向东(<a href="http://github.com/dongxiang/">@dongxiang</a>)]</address>
...
[Scala][Scala] 是Twitter所使用的主要程序开发语言之一。 我们（Twitter）软件基础架构的相当部分就是使用Scala编写，此外[一系列大规模的软件库](http://github.com/twitter/)为我们所用。Scala高效，是个重量级的开发语言，经验告诉我们开发时务必格外小心。有哪些陷阱？我们分别采用和规避了那些特性？ 何时采用“纯粹函数式编程风格”？什么时候避免这样做？换句话说，如何才能高效的使用这门语言？这份指南试图把我们的经验萃取成短文，为大家提供一系列*最佳实践*。我们主要使用Scala来开发组成Twitter分布式系统的高容量服务，所以我们的建议无疑偏重于此，但是这里的大部分建议都适用于其他场景。这些建议并不是铁律，但有相当的可信度。

Scala提供了许多用于简洁表达的工具。简洁的代码，要求的阅读量少，往往理解起来也快，换言之简洁的代码使得表达更加清晰。 但是简洁也可能适得其反，所以在保持代码正确性的同时，也要考虑可读性.

总而言之, *使用Scala编程*. 你不是在使用Java, Haskell, 或者Python; 使用Scala完成的程序和上述三者截然不同。 要有效的使用一门编程语言，你必须使用这门语言特有的方式来描述问题。把一个Java程序强制转换成Scala是徒劳的，因为这样完成的程序在大多数方面无法和原来的程序相比。

此文当并不是对Scala的介绍，我们假设读者已经熟悉这么语言。如果要学习Scala，请参考以下资源:
